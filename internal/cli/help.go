...
var commandDocs = []commandDoc{
	...
	},
	{
		name:    "changeradar",
		group:   groupAnalyze,
		summary: "Pull Request risk scoring, blast radius analysis, and test suite selection",
		usage:   []string{"entire graph changeradar [--base <rev>] [--head <rev>] [--format text|markdown|json] [--min-risk low|medium|high|critical] [--repo path]"},
		long:    "Combines git diff with whole-repo semantic impact analysis to assess the risk of a pull request or branch. Calculates a multi-factor risk score (0-100), identifies direct & transitive callers, detects external API routes affected, flags untested blast-radius blindspots, and selects the exact test suite to run before merging.",
		flags: []flagDoc{
			{name: "--base", arg: "rev", def: "main", desc: "Base revision or branch"},
			{name: "--head", arg: "rev", def: "HEAD", desc: "Head revision to compare"},
			{name: "--worktree", desc: "Include uncommitted working tree changes"},
			{name: "--format", arg: "text|markdown|json", def: "text", desc: "Output presentation format"},
			{name: "--min-risk", arg: "all|low|medium|high|critical", def: "all", desc: "Filter by minimum risk severity"},
			{name: "--depth", arg: "1|2", def: "2", desc: "Caller blast-radius traversal depth"},
			{name: "--limit", arg: "n", def: "15", desc: "Max callers listed per symbol"},
			{name: "--repo", arg: "path", desc: "Repository path (default: current repo)"},
			{name: "--profile", arg: "syntax-only|fast|full", def: "full", desc: "Snapshot indexing depth"},
		},
		examples: []string{
			"entire graph changeradar --base main --head HEAD",
			"entire graph changeradar --format markdown",
			"entire graph changeradar --format json --min-risk high",
		},
	},
	{
		name:    "diff-impact",
